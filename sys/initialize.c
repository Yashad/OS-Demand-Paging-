...
//Inititialize the bsm and frame table
	init_bsm();
	init_frm();
//set page fault ISR
	set_evec(14, pfintr);
//Initialize Global page table
	init_GlobalPageTable(); 
//PD for NULL process
	get_pageDirectory(NULLPROC);
	write2CR3(NULLPROC);
	enable_paging();
...
/*SC demand paging parameter for NULL process*/
	head_SC->frameID =-1;
	head_SC->nextFrame = tail_SC;
